...
ceo_rss_url = st.sidebar.text_input("公司/CEO 追蹤 (如官方 X)", value='https://news.google.com/rss/search?q="Ondas+Holdings"+OR+"ONDS"+OR+"Eric+Brock+Ondas"&hl=en-US&gl=US&ceid=US:en')
                ...
                try:
                    ...
                    if not available_models:
                        ...
                        if generate_regular:
                            ...
                            prompt = f"""你是一位頂尖的量化分析師。你的說話風格「斯文專業、條理清晰」，同時又兼具「詼諧幽默」，擅長用有趣且貼切的日常比喻來解釋枯燥的財報數據與市場現象。
請「完全捨棄」說教、倚老賣老或長輩對晚輩的語氣，請把我當作平起平坐的聰明投資夥伴來對話。
...
### 1. 📈 技術面與產業強弱診斷
請結合 RSI、MACD 判斷趨勢強弱，並根據「對手今日表現」比較個股相對其雷達名單的強弱勢。請適度發揮你的幽默感，用生動的比喻來形容現在的線型（例如：超買像是喝了太多雙倍濃縮咖啡心跳加速、跌破支撐像是沒踩剎車）。

### 2. 📰 基本面與社群情報提煉
綜合外電新聞、官方公告與獨家社群觀點，提煉出推動股價的核心基本面邏輯。

### 3. 🎯 全局戰略綜合決策
將上述的精準產業環境、技術、籌碼與情報完美融合，給出客觀且具體的短線觀察重點與操作建議。語氣要保持專業斯文，就像是我們在高級咖啡廳裡輕鬆但深度地討論一筆即將進場的交易。
"""
                        else:
